...
import { OrbitControls } from "three/examples/jsm/controls/OrbitControls.js";
    ...
    // Calculate bounds for centering and path calculation
    ...
    const [startRoom, setStartRoom] = useState<Feature | null>(null);
    const [endRoom, setEndRoom] = useState<Feature | null>(null);
    ...
    // Calculate path between two rooms
    const calculatePath = useCallback(() => {
        if (!startRoom || !endRoom || !sceneRef.current) return;

        const startCoords = getCentroid(startRoom.geometry.coordinates[0]);
        const endCoords = getCentroid(endRoom.geometry.coordinates[0]);
        ...
        const points = [
        new THREE.Vector3((startCoords[0] - centerX) * scale, 5, -(startCoords[1] - centerY) * scale),
        new THREE.Vector3((endCoords[0] - centerX) * scale, 5, -(endCoords[1] - centerY) * scale),
        ];
        ...
        if (pathLineRef.current) sceneRef.current.remove(pathLineRef.current);
        ...
        cameraRef.current.position.set(midPoint.x, 1000, midPoint.z + 500);
    ...
    useEffect(() => {
        ...
        scene.fog = new THREE.Fog(0xf5f7fa, 1000, 3000);
        ...
        minimapRenderer.domElement.style.border = "2px solid #e2e8f0";
        minimapRenderer.domElement.style.borderRadius = "8px";
        minimapRenderer.domElement.style.boxShadow = "0 2px 8px rgba(0,0,0,0.1)";
        ...
        if (showLabels) {
            ...
            label.innerHTML = `
            <div class="room-label flex items-center space-x-1.5">
                <span class="room-icon text-sm">${getRoomIcon(normalizedType)}</span>
                <span class="room-name font-medium text-sm">${roomName}</span>
                ${capacity !== "N/A" ? `<span class="room-capacity text-xs text-gray-500">(${capacity})</span>` : ""}
            </div>
            `;
        ...
        const gridHelper = new THREE.GridHelper(6000, 60, 0xe2e8f0, 0xf5f7fa);
        ...
        labelsRef.current.forEach(label => {
            ...
            if (vector.z < 1) {
                ...
                const opacity = showLabels ? Math.max(0.4, Math.min(1, (2000 - distance) / 1500)) : 0;
                label.style.opacity = opacity.toString();
                label.style.visibility = opacity > 0.4 ? "visible" : "hidden";
            } else {
        ...
        labelsRef.current = [];
        if (pathLineRef.current && sceneRef.current) sceneRef.current.remove(pathLineRef.current);
        renderer.dispose();
        if (minimapRenderer) {
    ...
    const focusOnRoom = (feature: Feature) => {
        if (cameraRef.current && controlsRef.current) {
        const centroid = getCentroid(feature.geometry.coordinates[0]);
        const bounds = calculateBounds(geoJson.features);
        const centerX = (bounds.minX + bounds.maxX) / 2;
        ...
        cameraRef.current.position.set(x, 500, z + 500);
    ...
    return (
        <div className="relative w-full h-screen bg-gradient-to-br from-gray-50 to-gray-200 overflow-hidden font-sans">
        {error && (
            <div className="absolute inset-0 bg-red-50 bg-opacity-95 flex items-center justify-center z-50">
            <div className="text-center p-6 bg-white rounded-xl shadow-lg border border-red-200">
                <p className="text-red-600 font-semibold text-lg mb-3 flex items-center">
        ...
        {isLoading && (
            <div className="absolute inset-0 bg-white bg-opacity-95 backdrop-blur-md flex items-center justify-center z-50">
            <div className="text-center">
                <div className="relative">
                <div className="animate-spin rounded-full h-20 w-20 border-4 border-t-blue-500 border-blue-200 mx-auto mb-4"></div>
                <Building className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 h-8 w-8 text-blue-500" />
                </div>
                <p className="text-gray-800 font-semibold text-lg">Loading Floor Plan...</p>
            </div>
            </div>
        )}

        <div className="absolute top-0 left-0 right-0 bg-white/90 backdrop-blur-lg border-b border-gray-200 z-30">
            ...
            <div className="flex items-center space-x-4">
                ...
                <h1 className="text-xl font-bold text-gray-800">Office Navigator</h1>
                </div>
            </div>
            <div className="flex items-center space-x-2">
                <button
                ...
                >
                {viewMode === "3d" ? <Grid3X3 className="h-5 w-5 text-gray-600" /> : <Move3D className="h-5 w-5 text-gray-600" />}
                <span className="absolute left-full ml-2 top-1/2 transform -translate-y-1/2 bg-gray-800 text-white text-xs rounded py-1 px-2 opacity-0 group-hover:opacity-100 transition-opacity">
                ...
                >
                {showLabels ? <Eye className="h-5 w-5 text-gray-600" /> : <EyeOff className="h-5 w-5 text-gray-600" />}
                <span className="absolute left-full ml-2 top-1/2 transform -translate-y-1/2 bg-gray-800 text-white text-xs rounded py-1 px-2 opacity-0 group-hover:opacity-100 transition-opacity">
        ...
        <div
            className={`absolute top-16 left-0 bg-white/95 backdrop-blur-lg rounded-r-xl shadow-xl p-5 z-20 transition-all duration-300 ease-in-out ${
        ...
        >
            ...
            <button
                ...
                className="p-2 rounded-lg hover:bg-gray-100 transition-colors w-full"
            ...
            <>
                ...
                className="w-full p-2 mb-4 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm bg-gray-50"
                aria-label="Search rooms"
                />
                <div className="mb-4">
                <button
                    onClick={() => setShowFilters(!showFilters)}
                    className="flex items-center space-x-1.5 text-sm font-semibold text-gray-700 hover:text-blue-600 transition-colors"
                ...
                {showFilters && (
                    ...
                    <select
                        ...
                        className="w-full p-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm bg-gray-50"
                ...
                <label className="block text-sm font-semibold text-gray-700">Start Room</label>
                <select
                    value={startRoom ? startRoom.properties.name || normalizeRoomType(startRoom.properties.type) : ""}
                    onChange={(e) => {
                    const room = filteredRooms.find(
                        r => r.properties.name === e.target.value || normalizeRoomType(r.properties.type) === e.target.value
                    );
                    setStartRoom(room || null);
                    if (room) focusOnRoom(room);
                    }}
                    className="w-full p-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm bg-gray-50"
                    aria-label="Select start room"
                >
                    <option value="">Select Start Room</option>
                    {filteredRooms.map(feature => (
                    <option key={feature.properties.name || feature.properties.type + feature.geometry.coordinates[0][0].toString()} value={feature.properties.name || normalizeRoomType(feature.properties.type)}>
                        {feature.properties.name || normalizeRoomType(feature.properties.type)}
                    </option>
                    ))}
                </select>
                <label className="block text-sm font-semibold text-gray-700 mt-2">End Room</label>
                <select
                    value={endRoom ? endRoom.properties.name || normalizeRoomType(endRoom.properties.type) : ""}
                    onChange={(e) => {
                    const room = filteredRooms.find(
                        r => r.properties.name === e.target.value || normalizeRoomType(r.properties.type) === e.target.value
                    );
                ...
                {startRoom && endRoom && (
                    ...
                    Path from <span className="font-medium">{startRoom.properties.name || normalizeRoomType(startRoom.properties.type)}</span> to <span className="font-medium">{endRoom.properties.name || normalizeRoomType(endRoom.properties.type)}</span>.
                    </div>
                )}
                </div>
            </>
        ...
        <div className="absolute top-20 right-6 bg-white/95 backdrop-blur-lg rounded-xl shadow-xl p-4 z-10 max-w-sm">
            ...
            <div className="flex justify-between items-center">
                ...
                <span className="font-medium text-blue-600">{geoJson.features.length}</span>
            </div>
            <div className="flex justify-between items-center">
                <span className="text-gray-600 flex items-center">
                ...
                <span className="font-medium text-green-600">{Object.keys(roomStats).length}</span>
            </div>
            <div className="flex justify-between items-center">
                <span className="text-gray-600 flex items-center">
        ...
        {hoveredRoom && (
            <div className="absolute bottom-6 left-6 bg-white/95 backdrop-blur-lg rounded-lg shadow-md p-3 z-10">
            <p className="text-sm font-medium text-gray-800">Hovered: {hoveredRoom}</p>
            </div>
        )}
